...
class TestNamespaceRegistration:
    ...
    def test_all_namespaces_registered(self, test_app):
        ...
        if not api_found:
            ...
            found_endpoints = [
                ep for ep in essential_endpoints if any(ep in endpoint for endpoint in endpoint_names)
            ]
            api_found = len(found_endpoints) > 0
            namespaces_found = found_endpoints

        # Au minimum, on doit avoir trouvé quelque chose
        assert api_found or len(namespaces_found) > 0, f"Aucune API ou namespace trouvé. Endpoints: \
            {[rule.endpoint for rule in test_app.url_map.iter_rules()]}"
...
class TestAPIConfiguration:
    ...
    def test_api_security_configuration(self, test_app):
        ...
        if hasattr(test_app, 'extensions'):
            ...
            jwt_configured = 'flask-jwt-extended' in test_app.extensions or any(
                'jwt' in ext.lower() for ext in test_app.extensions.keys()
            )
